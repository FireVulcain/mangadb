    ...
    <div class="select-wrap">
      <div class="select" @click="openFilter = !openFilter" v-on-clickaway="clickAway">
        <div class="value-wrap">
          <div
            class="placeholder"
            v-if="!selectedGenres.genres.length && !selectedGenres.tags.length"
          >Any</div>
          <div class="tags" v-if="selectedGenres.genres.length + selectedGenres.tags.length <= 1">
            <div
              class="tag"
              :key="index"
              v-for="(genre, index) in selectedGenres.genres"
            >{{ genre }}</div>
          </div>
          <div class="tags" v-if="selectedGenres.genres.length + selectedGenres.tags.length <= 1">
          ...
          <div class="tags" v-else>
            <div
              class="tag"
              v-if="selectedGenres.genres.length"
            >{{ selectedGenres.genres[0].toLowerCase() | capitalize }}</div>
            <div
              class="tag"
              v-else-if="selectedGenres.tags.length"
            >{{ selectedGenres.tags[0].toLowerCase() | capitalize }}</div>
        ...
        <section class="ps-container scroll-wrap ps">
          <div class="option-group">
            ...
            <div
              ...
              @click="toggleFilter(genre, 'genres'); open = false; "
            >
              <div class="label">
          ...
          <div class="option-group">
            ...
            <div
              ...
              @click="toggleFilter(tag.name, 'tags'); open = false; "
            >
              <div class="label">
    ...
    toggleFilter(filter, type) {
      ...
      if (type === "genres") {
        if (this.selectedGenres.genres.includes(filter)) {
          return (this.selectedGenres.genres = this.selectedGenres.genres.filter(
        ...
        if (this.selectedGenres.tags.includes(filter)) {
          return (this.selectedGenres.tags = this.selectedGenres.tags.filter(
            filters => filters !== filter
          ));
        }
