        ...
        <div class="value-wrap">
          ...
          <div class="tags" v-if="selectedFormat.length <= 1">
            <div
              class="tag"
              :key="index"
              v-for="(filter, index) in selectedFormat"
            >{{ filter.toLowerCase().replace(/_/g, ' ') }}</div>
          </div>
          <div class="tags" v-else-if="selectedFormat.length > 1">
            <div class="tag">{{ selectedFormat[0].toLowerCase().replace(/_/g, ' ') }}</div>
        ...
        <section class="ps-container scroll-wrap ps">
          <div class="option-group">
            <div
              ...
              @click="toggleFilter(format.id); open = false; "
            >
              <div class="label">
  ...
  methods: {
    ...
    toggleFilter(filter) {
      if (this.selectedFormat.includes(filter)) {
        return (this.selectedFormat = this.selectedFormat.filter(
