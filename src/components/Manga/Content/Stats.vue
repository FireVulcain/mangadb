    ...
    <div class="rankings-wrap" v-if="rankings.length">
      ...
      <div class="rankings graph">
        <div
          ...
          :class="ranking.type.toLowerCase()"
        >
      ...
      <div class="status-distribution content-wrap">
        <div class="statuses">
          <div class="status" :key="index" v-for="(distribution, index) in distributionStatus">
            <div
              class="name"
              :style="`background-color: ${colorStats[index]}`"
            >{{distribution.status.toLowerCase()}}</div>
    ...
    fetchStats() {
      axios({
        ...
        .then(result => {
          ...
          this.distributionStatus = result.data.data.Media.distribution.status.sort(
